...
import { saveConsumableEvent } from "../../../../src/v3/purchasely-webhook/consumables-webhook-event.controller";
...
test("Events Service Creation method is invoked", async t => {
  ...
  t.plan(1);
  await saveConsumableEvent(service)(event);
});
...
test("Events Service Creation method is successfully invoked with a consumbable event", async t => {
  const event = defaultWebhookEvent();
  const factoryService = servicesFactory().events;
  const service = {
    ...factoryService,
