...
export const PurchaselyServices =
  (purchaselyConfig: PurchaselyConfigInterface) =>
    (auth: Firebase.auth.Auth, db: FirebaseFirestore.Firestore): {
      ...
      events: servicesSetup<EventsInterface>(purchaselyConfig[2].destinations.purchaselyEvents)((collectionName) => EventsInstance(collectionName)(db)),
      firebaseCustomClaims: FirebaseCustomClaimsServiceInstance(auth),
      logs: PurchaselyLoggingService(),
      consumables: servicesSetup<ConsumablesInterface>(purchaselyConfig[2].destinations.purchaselyConsumables)((collectionName) => ConsumablesInstance(collectionName)(db)),
      nonConsumables: servicesSetup<NonConsumablesInterface>(purchaselyConfig[2].destinations.purchaselyNonConsumables)((collectionName) => NonConsumablesInstance(collectionName)(db)),
      subscriptions: servicesSetup<SusbscriptionsInterface>(purchaselyConfig[2].destinations.purchaselySubscriptions)((collectionName) => SubscriptionsInstance(collectionName)(db)),
    });